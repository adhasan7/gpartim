            ...
            <div class="row" style='margin-right:10px; margin-left:10px;'>
                <?php 
                    $top_kategori = $this->db->query("SELECT * FROM (SELECT a.*, b.jumlah FROM
                                        ...
                                        (SELECT z.id_kategori_produk, COUNT(*) jumlah FROM rb_penjualan_detail y JOIN rb_produk z ON y.id_produk=z.id_produk GROUP BY z.id_kategori_produk HAVING COUNT(z.id_kategori_produk)) as b on a.id_kategori_produk=b.id_kategori_produk) as x ORDER BY x.jumlah 
                                        ");
                        ...
                        echo "<div class='col-xl-2 col-lg-3 col-md-4 col-sm-4 col-3' style='padding-right:10px; padding-left:10px;'>
                                <div class='ps-block--category' style='padding:30px 10px; border-radius:20px; border:1px solid #fff; color:#000; margin-bottom:50px;'>
                                    <a style='margin-top:15px; height:80%' class='ps-block__overlay' href='".base_url()."produk/kategori/$row[kategori_seo]'>
                                        $icon <p style='font-size:11px; padding-top:0px; line-height:1.1em'>$row[nama_kategori] </p>
                                    </a>
                                </div>
                              </div>";
                    }
    ...
    <div class="ps-top-categories d-none d-sm-block">
        <div class="ps-container">
            ...
            <div class="row">
                <?php 
                    $top_kategori = $this->db->query("SELECT * FROM (SELECT a.*, b.jumlah FROM
                                        (SELECT * FROM rb_kategori_produk WHERE NOT urutan = '0') as a LEFT JOIN

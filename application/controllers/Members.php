...
The code will expire in 10 minutes, If you have not tried to login, ignore this message.";
	...
	function kategori()
	{
		...
		$config['base_url'] = base_url() . 'members/kategori/' . $this->uri->segment(3);
		$config['total_rows'] = $jumlah;
		$config['per_page'] = 18;
		if ($this->uri->segment('4') == '') {
			$dari = 0;
		...
		if (is_numeric($dari)) {
			...
			$this->pagination->initialize($config);
			$this->template->load(template() . '/template', template() . '/reseller/view_kategori_all2', $data);
		} else {
			redirect('main');
		}
		//}
	...
	// Fungsi ini akan menangani form login ojek
	// function ojek_login_process(){
	//     if ($this->input->post('submit')) {
	//         // Logika untuk memproses login ojek setelah tombol submit ditekan
	//         // ...
	//     } else {
	//         // Redirect ke halaman login ojek jika tidak ada data yang dikirim
	//         redirect('members/otp');
	//     }
	// }
		...
		if (isset($_POST['submit'])) {
			...
			if ($status >= 1) {
				...
				if ($cek_produk->num_rows() >= $cekpa['max_produk']) {
					...
					$data = array(
						'id_kategori_produk' => cetak($this->input->post('a')),
	...
	function tambah_pembelian()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if ($this->session->idp == '') {
				...
				$data = array(
					'kode_transaksi' => $kode_transaksixe,
					'id_pembeli' => reseller($this->session->id_konsumen),
					'id_penjual' => '1',
					'status_pembeli' => 'reseller',
					'status_penjual' => 'admin',
	...
	function konfirmasi_pembayaran()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if ($hasil['file_name'] == '') {
				$data = array(
					'id_penjualan' => cetak($this->input->post('id')),
	...
	function detail_penjualan()
	{
		...
		if (isset($_POST['submit'])) {
			$data1 = array(
				'no_resi' => cetak($this->input->post('no_resi')),
				'proses' => '3'
			);
	...
	function tambah_penjualan()
	{
		...
		if (isset($_POST['submit1'])) {
			if ($this->session->idp == '') {
				$data = array(
					'kode_transaksi' => cetak($this->input->post('a')),
	...
	function edit_penjualan()
	{
		...
		if (isset($_POST['submit1'])) {
			...
			if ($this->input->post('dd') > $stok) {
				...
				if ($this->input->post('idpd') == '') {
					$data = array(
						'id_penjualan' => cetak($this->input->post('idp')),
						'id_produk' => cetak($this->input->post('aa')),
						'jumlah' => cetak($this->input->post('dd')),
						'harga_jual' => cetak($this->input->post('bb')),
						'satuan' => cetak($this->input->post('ee'))
	...
	function upgrade()
	{
		...
		if (isset($_GET['paket'])) {
			...
			if ($rowp['status'] == 'Y') {
				...
				$data = array(
					'id_paket' => cetak($this->input->get('paket')),
					'id_reseller' => reseller($this->session->id_konsumen),
	...
	function buat_toko()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if ($cek_user_exist->num_rows() >= 1) {
				$user_reseller = seo_title(cetak($this->input->post('c'))) . rand(100, 999);
			} else {
				$user_reseller = seo_title(cetak($this->input->post('c')));
			}
			if ($hasil['file_name'] == '') {
				$data = array(
					'id_konsumen' => cetak($id),
	...
	function ojek_online()
	{
		...
		$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array();
		$this->template->load(template() . '/template', template() . '/ojek/view_ojek', $data);
	}
	// make new order
	function pesanan()
	{
		...
		$this->template->load(template() . '/template', template() . '/ojek/view_order', $data);
	}
}
